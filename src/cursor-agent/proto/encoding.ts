...
export function encodeStringField(fieldNumber: number, value: string): Uint8Array {
  ...
  const fieldTag = encodeVarint((fieldNumber << 3) | 2); // wire type 2 = length-delimited
  const encoded = new TextEncoder().encode(value);
  const length = encodeVarint(encoded.length);

  const result = new Uint8Array(fieldTag.length + length.length + encoded.length);
  result.set(fieldTag, 0);
  result.set(length, fieldTag.length);
  result.set(encoded, fieldTag.length + length.length);
...
export function encodeUint32Field(fieldNumber: number, value: number): Uint8Array {
  ...
  const fieldTag = encodeVarint((fieldNumber << 3) | 0); // wire type 0 = varint
  const encoded = encodeVarint(value);

  const result = new Uint8Array(fieldTag.length + encoded.length);
  result.set(fieldTag, 0);
  result.set(encoded, fieldTag.length);
...
export function encodeMessageField(fieldNumber: number, data: Uint8Array): Uint8Array {
  ...
  const result = new Uint8Array(fieldTag.length + length.length + data.length);
  result.set(fieldTag, 0);
  result.set(length, fieldTag.length);
  result.set(data, fieldTag.length + length.length);
...
export function encodeBoolField(fieldNumber: number, value: boolean): Uint8Array {
  const fieldTag = encodeVarint((fieldNumber << 3) | 0); // wire type 0 = varint
  const result = new Uint8Array(fieldTag.length + 1);
  result.set(fieldTag, 0);
  result[fieldTag.length] = value ? 1 : 0;
  return result;
}
...
export function encodeDoubleField(fieldNumber: number, value: number): Uint8Array {
  const fieldTag = encodeVarint((fieldNumber << 3) | 1); // wire type 1 = 64-bit
  const buffer = new ArrayBuffer(fieldTag.length + 8);
  const view = new DataView(buffer);
  new Uint8Array(buffer).set(fieldTag, 0);
  view.setFloat64(fieldTag.length, value, true); // little-endian
